...
```env
# Hide DeepSeek reasoning-content recovery notices (default: false)
...
- **Local token counting** — `/v1/messages/count_tokens` returns an approximate count without calling the upstream API. Reflects compaction when enabled.
- **Optional memory compaction** — preserves important session context by extractively summarizing older messages. Tracks DeepSeek reasoning recovery and activates safe mode to reduce proxy cache misses.
...
|---------|-------------|
| `[deepseek-cursor-proxy] Refreshed reasoning_content history.` in output | Proxy reasoning recovery notice — hidden by default; see [troubleshooting](examples/troubleshooting.md#deepseek-cursor-proxy-refreshed-reasoning_content-history) |
...
## Bridge Memory Compaction

Long Claude Gateway sessions can reach the upstream context limit or trigger repeated DeepSeek `reasoning_content` recovery, causing the model to forget prior context. The optional **Bridge Memory Compaction** feature addresses both problems.

### What It Does

When enabled, the bridge estimates the size of each incoming `/v1/messages` request. If it exceeds `BRIDGE_COMPACTION_TRIGGER_TOKENS`, the oldest messages are replaced with a structured extractive summary that preserves:
...
- URLs and references
- Decisions and plans
- Tool names and their results (as text)
- Code snippets

The most recent `BRIDGE_COMPACTION_KEEP_RECENT_MESSAGES` messages are kept verbatim so active conversation context is not lost.

### Reasoning Recovery Tracking

DeepSeek's thinking mode requires `reasoning_content` to be passed back on subsequent turns after tool-call reasoning. The `deepseek-cursor-proxy` caches and reinjects this content. If the bridge changes old message content (through compaction or sanitization), the proxy cache can miss, triggering a "refreshed reasoning_content history" recovery event. Each recovery may drop older tool-call context.

The bridge tracks these events per session. When repeated recovery is detected:

- **Safe mode** activates automatically (in `auto` mode) or can be forced (`always` mode).
- In safe mode, compaction is more conservative: old tool-call episodes are always summarized as text rather than kept as raw `tool_calls`, reducing the chance of proxy cache mismatches.
- Fewer recent messages are kept raw to minimize old tool_calls in the upstream request.
...
BRIDGE_COMPACTION_TRIGGER_TOKENS=180000
BRIDGE_COMPACTION_TARGET_TOKENS=120000
BRIDGE_COMPACTION_KEEP_RECENT_MESSAGES=20
...
- **Disabled by default.** Existing behavior is unchanged when compaction is off.
- **Best-effort.** Compaction cannot preserve every token perfectly. It preserves important session facts, decisions, file paths, commands, bugs, and current goals.
- **Local storage.** Memory summaries are stored as `.txt` files in `.bridge-memory/` (automatically gitignored). Do not commit this directory.
- **No upstream model call.** The summary is generated deterministically from the existing messages — no API call is made to summarize.
- **Not perfect memory.** This feature reduces forgetting caused by context pressure and reasoning recovery, but it cannot eliminate it.

### Diagnostics

Access session diagnostics at:

```
GET /v1/memory/inspect?fingerprint=<session_fingerprint>
Authorization: Bearer sk-local-zen
```

Returns recovery counts, safe mode status, and whether a persisted summary exists — but no raw prompt content or secrets.
...
│   ├── compaction.py               # Optional memory compaction + recovery tracking
