...
## Future Improvement: Rolling Memory Summary

If `deepseek-cursor-proxy` repeatedly triggers reasoning_content recovery events, the model may lose user-visible context. A bridge-side rolling memory summary could help recover context by injecting a brief conversation summary into each request. This is not enabled by default because it has privacy and correctness trade-offs — the summary is generated by an external model and may introduce artifacts.
